<template>
  <div class="layout" v-if="studentStore.profile">
    <!-- LEFT PANEL -->
    <div class="left-card">
      <div class="avatar-wrapper">
        <!-- <img
          :src="
            studentStore.profile.avatar || 'https://i.pravatar.cc/160?img=32'
          "
          alt="Profile photo"
          class="avatar-image"
        /> -->
        <img :src="avatarUrl" alt="Profile photo" class="avatar-image" />
        <input
          ref="avatarInput"
          type="file"
          accept="image/*"
        ...
        <button
          ...
          title="Delete photo"
          :disabled="avatarDeleting || !studentStore.profile.avatar"
          @click="deleteAvatar"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
      ...
      <div class="user-name">
        {{ studentStore.profile.firstName }} {{ studentStore.profile.lastName }}
      </div>
    ...
    <div class="right-card">
      ...
      <div class="info-grid">
        <div class="info-item">
          ...
          <div class="info-value">{{ studentStore.profile.firstName }}</div>
        </div>
        ...
        <div class="info-item">
          ...
          <div class="info-value">{{ studentStore.profile.lastName }}</div>
        </div>
        ...
        <div class="info-item">
          ...
          <div class="info-value">{{ studentStore.profile.address }}</div>
        </div>
        ...
        <div class="info-item">
          ...
          <div class="info-value">{{ studentStore.profile.phone }}</div>
        </div>
...
const avatarUrl = computed(() => {
  if (!studentStore.profile?.avatar) {
    return "https://i.pravatar.cc/160?img=32";
  }
  return `http://quiz_management_system.tdomain.work.gd${studentStore.profile.avatar}?t=${imageRefresh.value}`;
});
...
const handleChangePassword = async () => {
  ...
  try {
    ...
    await studentStore.changePassword(
...
const uploadAvatar = async (event) => {
  ...
  if (!file) return;
  try {
    await studentStore.uploadAvatar(file);
...
const deleteAvatar = async () => {
  if (!studentStore.profile?.avatar) return;

  try {
    avatarDeleting.value = true;

    await studentStore.deleteAvatar();
...
const handleDeleteAccount = async () => {
  ...
  try {
    ...
    await studentStore.deleteAccount(deleteAccountForm.value.password);
    ...
    deleteAccountError.value =
      studentStore.error || err.message || "លេខសម្ងាត់មិនត្រឹមត្រូវ!";
  } finally {
