        ...
        <div class="search-wrapper flex-grow-1" style="max-width: 380px;">
          <div class="d-flex align-items-center gap-2 p-2 rounded-pill"
            ...
            <input type="text" :value="modelValue" @input="$emit('update:modelValue', $event.target.value)"
              ...
              style="outline: none; font-size: 0.85rem; color: var(--txt);" placeholder="ស្វែងរកការប្រឡង...">
          </div>

          <div v-if="modelValue && filteredExams.length > 0" class="suggestions-dropdown shadow-sm">
            <router-link v-for="exam in filteredExams.slice(0, 5)" :key="exam.id"
              :to="`/teacher/room-management/${exam.room_id || '0'}/exams/${exam.id}`" class="suggestion-item">
